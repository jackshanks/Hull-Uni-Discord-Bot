...
class RoleCommands(BaseCog):
    ...
    async def assign_game(self, interaction: Interaction):
        await interaction.response.send_message("Select new colour", view=DropdownView(interaction, role_type="game"), ephemeral=True)

    @nextcord.slash_command(name="remove-game", description="Give yourself game roles!", guild_ids=Config().guild_ids)
    async def assign_game(self, interaction: Interaction):
        await interaction.response.send_message("Select new colour", view=DropdownView(interaction, role_type="game"), ephemeral=True)
